...
export function SolutionSubmission({
  ...
  const [isInvestigating, setIsLoading] = useState(false);
    ...
    try {
      ...
      if (isAnswerCorrect) {
        ...
        if (user) {
          ...
          const { data: userInfo, investigationError: fetchError } = await supabase
            .from("user_info")
          ...
          if (!completedCases.includes(caseData.id)) {
            ...
            const { investigationError: updateError } = await supabase.rpc(
              "increment_user_xp",
              {
                user_id: user.id,
      ...
      setError(
        err instanceof Error
          ? err.message
          : "An investigationError occurred while updating progress"
      );
      console.investigationError("Error updating solved cases:", err);
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="flex justify-end">
              <button
                ...
                className={`
                  ...
                  ${isInvestigating ? "opacity-75 cursor-not-allowed" : ""}
                `}
              >
                {isInvestigating ? (
                  <>
