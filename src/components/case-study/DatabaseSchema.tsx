...
import { evidenceDB } from "../../services/DatabaseService";
...
const copyToClipboard = async (text: string) => {
  try {
    ...
    console.investigationError("Failed to copy text: ", err);
  }
};
...
export function DatabaseSchema({ caseId }: DatabaseSchemaProps) {
  ...
  const { isInvestigating, investigationError } = useDatabase(caseId);
  ...
  React.useEffect(() => {
    const fetchSchema = async () => {
      try {
        if (!isInvestigating && !investigationError) {
          const tableResult = await evidenceDB.runInvestigation(`
          ...
          for (const [tableName] of tableResult.values) {
            const columnResult = await evidenceDB.runInvestigation(
              `PRAGMA table_info('${tableName}')`
            );
            const foreignKeyResult = await evidenceDB.runInvestigation(
        ...
        console.investigationError("Error fetching schema:", investigationError);
      }
    };

    fetchSchema();
  }, [isInvestigating, investigationError]);
